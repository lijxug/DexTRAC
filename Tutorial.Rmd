...
data_dir = '~/data/projects/09.Niehu_HBVHCC/'
...
#' @param ctrl_vec Named vector. Recording 'ctrl' or 'exp' information for each cell. Named by cellID, must be consistent with the rownames of `atbd_mt`
#'
#' @return
#'
estimate_truecounts = function(atbd_mt, ctrl_vec){
  
  stopifnot(all(names(ctrl_vec) == rownames(atbd_mt)))
...
tmp_plt
# ggsave(sprintf('%s/plots/barplot_nbinomrandom_example.pdf', data_dir), plot = tmp_plt, width = 8, height = 5)
# gpexpr_lst[[10]]$plot
```
...
pheatmap::pheatmap(
  ...
  breaks = breaks_vec,
  display_numbers = T,
  filename = sprintf('%s/plots/heatmap_HBVantigen_specificity.pdf', data_dir), 
  height = 10, width = 12,
  main = 'Enrichment'
)
pheatmap::pheatmap(
...
ggsave(filename = sprintf('%s/plots/piebatch_antigen_specificities.pdf', data_dir), height = 20, width = 20)
...
plt_OR_df = plt_OR_df %>% 
  mutate(affinity = ifelse(Fisher.OR > 10^logOR_cutoff, 'High', 'Low'))
...
# ggsave(filename = sprintf('%s/plots/barplot_ORcategory.pdf', data_dir), height = 4, width = 6)
```
...
ggsave(filename = sprintf('%s/plots/piebatch_antigen_affinities.pdf', data_dir), height = 10, width = 22)
...
ggsave(filename = sprintf('%s/plots/piebatch_topclonalClones.pdf', data_dir), height = 18, width = 22)
...
ggsave(filename = sprintf('%s/plots/pie_clone_%s_inPat_%s.pdf', data_dir, t_tcr_clone, clone2pat_vec[t_tcr_clone]), width = 6, height = 6)
...
plt_mt = bind_count_mt[names(flt_cloneid_vec)[flt_cloneid_vec == t_tcr_clone],] 
order_atbd = colSums(plt_mt) %>% sort(decreasing = T) %>% names
...
pdf(file = sprintf('%s/plots/heatmap_clone_%s_inpat_%s.pdf', data_dir, t_tcr_clone, clone2pat_vec[t_tcr_clone]), width = 10, height = 6)
ComplexHeatmap::pheatmap(
  t(plt_mt[, order_atbd]),
  show_rownames = T,
  show_colnames = F,
...
row_idx = rowSums(plt_mt) > 0
col_idx = colSums(plt_mt) > 0
ComplexHeatmap::pheatmap(
  t(plt_mt[row_idx, col_idx]),
  show_rownames = T,
  show_colnames = F,
  cluster_cols = F,
  cluster_rows = F,
...
pdf(file = sprintf('%s/plots/heatmap_clone_%s_inpat_%s_flt.pdf', data_dir, t_tcr_clone, clone2pat_vec[t_tcr_clone]), width = 10, height = 6)
ComplexHeatmap::pheatmap(
  t(plt_mt[row_idx, col_idx]),
  show_rownames = T,
  show_colnames = F,
...
meta_CD8_tbl %>% dplyr::filter(clone.id %in% signif_clonotypes2run) %>% 
  ...
  labs(x = "TCRβ v gene + j gene", y = 'Number of clonotypes', title = 'HBV-specific TCRs') + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
ggsave(filename = sprintf('%s/plots/barplot_TRBcombn_count.pdf', data_dir), width = 6, height = 6)
...
test_tbl = test_tbl %>% mutate(adj.p.val = p.adjust(p.val, 'BH')) %>% dplyr::arrange(p.val)
write_csv(test_tbl, file = sprintf('%s/plots/TRBcombn_significance.csv', data_dir))
...
# Storage
```{r}
# save.image('/home/jason/data/projects/09.Niehu_HBVHCC/tmp.RData')
# load('/home/jason/data/projects/09.Niehu_HBVHCC/tmp.RData')
```
